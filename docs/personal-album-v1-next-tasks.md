...
> **文档定位**：本文件是 V1 Ready 验收通过后的长期任务参考清单，记录了 P1-06/P1-07/P1-08（Viewer 性能）和 P2（分发与 SEO）等后期任务。
>
> **当前状态**：V1 Ready 已于 2026-09-12 签字验收通过，当前执行路线见 [`v1-post-one-month-roadmap.md`](v1-post-one-month-roadmap.md)
>
> **原规划日期**：2026-09-08
>
> **原基线**：`feat/gallery-workspace-slice` / `2358489`
>
> **用途**：本文件作为长期待办事项库，供产品规划和技术演进参考，不作为当前阶段的执行文档。

---

## 📌 文档状态说明

- ✅ **P0 任务（上线阻断）**：已全部完成（WP-6 至 WP-9、SC、M7.5）
- ✅ **P1 核心体验任务（P1-01 至 P1-05）**：已全部完成
- 🔄 **P1 Viewer 性能任务（P1-06/07/08）**：部分实现，纳入月度路线图持续优化
- ⏳ **P2 分享分发任务（P2-01/02/03）**：待评估优先级，按用户反馈排期

当前一个月（2026-09-17 至 2026-10-17）的具体执行计划见 **[`v1-post-one-month-roadmap.md`](v1-post-one-month-roadmap.md)**。
...
2. **以真实状态为准**：M7.1 配置版本化和 M7.2 TEXTURE 已完成真实验收；**M7.5 / V1 Ready 已于 2026-09-12 签字**（见 [`v1-ready-signoff.md`](v1-ready-signoff.md)）。M7.3/M7.4 属 V1 后演进。
...
- ✅ Admin UI 密码恢复表单（`OverviewView.vue` 忘记密码 + `ResetPasswordView.vue` 重置密码）与 Gallery 密码设置 UI（`ShareDeliveryPanel.vue`）已交付
- 生产 SMTP 邮件适配器（当前本地/测试环境通过日志输出安全重置令牌）
...
- ✅ 建立 `GalleryMetrics`，使用 Micrometer 暴露 `gallery.upload.accepted/rejected`、`gallery.task.failed`、`gallery.storage.put.error` 与 `gallery.public.access` 等业务指标。
- ✅ 完善结构化日志与全链路 `requestId` 追踪（`RequestIdFilter`、MDC 绑定、响应头 `X-Request-Id` 回传）。
- ✅ 建立故障排障与灾难恢复手册：[`docs/operations-recovery.md`](operations-recovery.md)，覆盖上传排障、队列积压、孤儿对象清理、数据库/对象存储备份还原与部署回滚。
- ✅ 记录灾难恢复与排障实战演练（含 requestId 定位、链接撤销、配额回滚与密码重置 4 项演练）。
...
- ✅ `GalleryPhotoGrid.vue` 完整接入工作区，保留 Dropzone 拖拽上传。
- ✅ 状态筛选：全部、已就绪、处理中、失败，状态角标全中文无英文泄漏。
- ✅ 多选与批量删除：浮动底栏、`ConfirmModal` 二次确认（「从展厅移除，不可撤销」），`Promise.allSettled` 并行软删除，汇总反馈并自动刷新。
- ✅ 标题编辑：Lightbox 内点击标题即席编辑，按 Enter 保存，调用 `PATCH /api/photos/{id}`。
- ✅ 照片排序：卡片下拉菜单与 Lightbox 内支持前移/后移，重新赋递增 sortOrder 并持久化更新。
- ✅ 失败项重试：筛选「失败」或浮动栏一键重试失败项，调用 `/api/photos/tasks/{taskId}/retry`。
- ✅ 设封面：卡片操作菜单与 Lightbox 内支持一键设为相册封面，封面徽章明确提示。
- ✅ 共享契约：`packages/gallery-contracts` 补齐 `UpdatePhotoRequest`。
...
- ✅ 新建 `ShareDeliveryPanel.vue` 与 `useShareDelivery.ts`，统一呈现访问模式、相册密码、访客下载权限、分享链接与已有链接管理。
- ✅ 访问模式清晰区分：公开（PUBLIC）、私密（PRIVATE）、密码（PASSWORD）。
- ✅ 相册密码管理：OWNER 可直接在分享面板设置、修改或清除访问密码，说明访客输入密码解锁 30 分钟临时会话。
- ✅ 访客下载权限：配置 schema 增加 `visitorAllowDownload: boolean`（默认 `false`），分享面板只读展示并引导修改；Viewer 端 Lightbox 根据配置安全显隐中等画质下载入口。
- ✅ 分享链接有效期：支持 7天/30天/90天/永久；交付确认态清晰展示生成结果、剩余时间、一键复制与使用说明。
- ✅ 已有链接列表与可观测：展示有效状态、创建时间、剩余有效时间、最近访问时间（未访问显示「尚未访问」）与二次确认撤销。
...
> **V1 Ready 当前路径（2026-09-10 固定）：** 不再扩展功能面，只闭环交付。详细任务见 [`superpowers/plans/2026-09-10-personal-album-v1-phase-3-implementation.md`](superpowers/plans/2026-09-10-personal-album-v1-phase-3-implementation.md)。
>
> ```text
> WP-6 整理照片 → WP-7 分享交付 → WP-9 用户自助 → Security Check → WP-8 可观测 → M7.5 回归
> ```
>
> **V1 Ready 标准：** 真实用户无需内部文档即可完成上传、整理、发布、分享、访问/下载；出问题可凭 requestId/指标/手册定位恢复。

### 批次 F：V1 Ready 闭环（✅ 已完成 2026-09-12）

```text
WP-6  P1-03  照片策展（筛选/批量删/标题/排序/重试/封面）  ✅
WP-7  P1-05  分享交付（链接/密码/下载权限；二维码可降级） ✅
WP-9  P0-03  忘记/重置密码 Admin UI                     ✅
SC    安全检查  IDOR、租户隔离、分享 Token、密码、上传边界 ✅
WP-8  P0-05  指标、结构化日志、恢复手册 + DR-05           ✅
M7.5  综合回归  仅回归，不新增功能                        ✅
```

出口：达到 V1 Ready 签字验收 — **已达成**（[`v1-ready-signoff.md`](v1-ready-signoff.md)）。
...
P1-02 相册总览和管理摘要        ✅
P1-04 发布中心和草稿预览        ✅
P1-03 照片记录与策展            → 批次 F / WP-6
P1-05 分享交付体验              → 批次 F / WP-7
```

### 批次 D：访客体验（移出 V1 Ready，V1 后执行）
...
```text
M7.5 综合回归              → 批次 F 最后一步（仅回归）
安全检查（IDOR/租户/Token）  → 批次 F / SC（独立轮次）
备份恢复演练               → 批次 F / WP-8 手册 + 演练记录
生产部署演练与回滚         → V1 Ready 签字后
移动端与低端设备测试       → V1 后 / P1-06 里程碑
```
